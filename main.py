...
def main():
    ...
    result = (
        shape
        .pipe(mirror_x)
        .pipe(lambda s: add_radial_noise(s, max_damage=0.2))
    )

    # 3) vykreslení
    d = Drawer()
    d.draw_shape(shape, color="blue", linewidth=1)  # původní tvar
    d.draw_shape(shape_before_transform, color="gray", linewidth=1) # před zásahem šumu    
    d.draw_points(point_centroid, color="red", size=50)  # těžiště    
    d.draw_shape(result, color="black")     # deformovaný
    d.show()
